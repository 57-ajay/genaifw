...
class AudioConfigService:
    ...
    def get_url(
        ...
        Priority order:
        1. If ENTRY intent and is_home=False and request_count > 1, return 'entry_request_count'
        2. If ENTRY intent and is_home=False, return 'entry_2' (overrides interaction_count)
        3. If interaction_count >= 5 and a '_short' version exists, return the short version
        4. Otherwise return the regular version
        ...
        Args:
            ...
            request_count: Number of requests made by user (optional)
        ...
        # Special case: ENTRY intent with multiple requests on non-home page
        if (
            ...
            # Use special audio for returning users on non-home pages
            entry_request_count_url = self.get_url_direct("entry_request_count")
            if entry_request_count_url:
                logger.debug(
                    f"Using entry_request_count audio for non-home with request_count={request_count}"
                )
                return entry_request_count_url
